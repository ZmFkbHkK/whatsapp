...
async function GroupUpdate(xdbotz, update, store, groupCache) {
	try {
		for (let n of update) {
...
async function GroupParticipantsUpdate(xdbotz, { id, participants, author, action }, store, groupCache) {
	try {
		...
		if (global.db.groups && global.db.groups[id] && store.groupMetadata && store.groupMetadata[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await xdbotz.profilePictureUrl(n, 'image');
				} catch {
...
async function LoadDataBase(xdbotz, m) {
	try {
...
async function MessagesUpsert(xdbotz, message, store, groupCache) {
	try {
		let botNumber = await xdbotz.decodeJid(xdbotz.user.id);
		const msg = message.messages[0];
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) {
			store.groupMetadata ??= await xdbotz.groupFetchAllParticipating().catch(e => ({}))
		}
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		if (!msg.key.fromMe && !msg.message && message.type === 'notify') return
		const m = await Serialize(xdbotz, msg, store, groupCache)
		require('../xdbotz')(xdbotz, m, message, store, groupCache);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			await xdbotz.appendResponseMessage(m, JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id);
		}
		if (global.db.set && global.db.set[botNumber] && global.db.set[botNumber].readsw) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await xdbotz.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) xdbotz.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await xdbotz.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(xdbotz, store) {
	xdbotz.serializeM = (m) => MessagesUpsert(xdbotz, m, store)
	
	xdbotz.decodeJid = (jid) => {
		if (!jid) return jid
		if (/:\d+@/gi.test(jid)) {
	...
	xdbotz.getName = (jid, withoutContact  = false) => {
		const id = xdbotz.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || xdbotz.groupMetadata(id) || {};
	...
	xdbotz.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await xdbotz.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await xdbotz.getName(i + '@s.whatsapp.net')}\nFN:${await xdbotz.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await xdbotz.getName(i + '@s.whatsapp.net')}\nFN:${await xdbotz.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/xdbotz_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		xdbotz.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
	}
	
	xdbotz.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await xdbotz.query({
			tag: 'iq',
			attrs: {
	...
	xdbotz.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return xdbotz.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted.expiration || 0 })
	}
	
	xdbotz.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return xdbotz.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return xdbotz.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return xdbotz.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('image')) {
				return xdbotz.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('video')) {
				return xdbotz.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('audio')) {
				return xdbotz.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			}
		}
	...
	xdbotz.sendGroupInvite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await xdbotz.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	xdbotz.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await xdbotz.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	xdbotz.sendTextMentions = async (jid, text, quoted, options = {}) => xdbotz.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	xdbotz.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return xdbotz.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
	}
	
	xdbotz.downloadMediaMessage = async (message) => {
	...
	xdbotz.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await xdbotz.downloadMediaMessage(message);
	...
	xdbotz.getFile = async (PATH, save) => {
		let res;
		let filename;
	...
	xdbotz.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: xdbotz.user.id, quoted: m.quoted });
		apb.key = m.key
		apb.key.fromMe = areJidsSameUser(m.sender, xdbotz.user.id);
		if (m.isGroup) apb.participant = m.sender;
		xdbotz.ev.emit('messages.upsert', {
			...m,
	...
	xdbotz.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await xdbotz.getFile(path, true);
		...
		let anu = await xdbotz.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	xdbotz.sendListMsg = async (jid, content = {}, options = {}) => {
					...
					interactiveMessage: proto.Message.InteractiveMessage.create({
						...
						header: proto.Message.InteractiveMessage.Header.fromObject({
							...
							...(media && typeof media === 'object' && Object.keys(media).length > 0 ? await generateWAMessageContent(media, {
								upload: xdbotz.waUploadToServer
							}) : {})
						}),
		...
		const hasil = await xdbotz.relayMessage(msg.key.remoteJid, msg.message, {
	...
	xdbotz.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	xdbotz.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const hasil = await xdbotz.query({
				tag: 'message',
				attrs: {
					to: key,
					type: 'reaction',
			...
			const msg = await generateWAMessageContent(media, { upload: xdbotz.waUploadToServer });
			const anu = await xdbotz.query({
				tag: 'message',
			...
			const _query = await xdbotz.query({
				tag: 'iq',
				attrs: {
					to: 's.whatsapp.net',
					type: 'get',
	...
	xdbotz.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: xdbotz.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await xdbotz.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (xdbotz.user && xdbotz.user.id) {
		const botNumber = xdbotz.decodeJid(xdbotz.user.id);
		if (global.db.set && global.db.set[botNumber]) {
			xdbotz.public = global.db.set[botNumber].public
		} else xdbotz.public = true
	} else xdbotz.public = true

	return xdbotz
}

async function Serialize(xdbotz, m, store, groupCache) {
	const botNumber = xdbotz.decodeJid(xdbotz.user.id)
	if (!m) return m
	...
	if (m.key) {
		...
		m.sender = xdbotz.decodeJid(m.fromMe && xdbotz.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await xdbotz.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = groupCache.get(m.chat))
			if (!metadata) {
				metadata = await xdbotz.groupMetadata(m.chat).catch(e => ({}))
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = xdbotz.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === xdbotz.decodeJid(xdbotz.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, xdbotz)
				return await Serialize(xdbotz, q, store, groupCache)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(xdbotz.decodeJid(m.msg?.contextInfo?.participant), xdbotz.decodeJid(xdbotz?.user?.id)),
			...
			m.quoted.download = () => xdbotz.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				xdbotz.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => xdbotz.downloadMediaMessage(m)
	
	m.copy = () => Serialize(xdbotz, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return xdbotz.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
		} else if (typeof content === 'string') {
			return xdbotz.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
		}
	}
